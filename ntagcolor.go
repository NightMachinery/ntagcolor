...
var tagStyleLookup = buildTagStyleLookup(tagStyles)

func buildTagStyleLookup(styles []tagStyle) map[string]tagStyle {
	lookup := make(map[string]tagStyle)
	for _, style := range styles {
		for _, name := range style.Names {
			lookup[strings.ToLower(name)] = style
		}
	}
	return lookup
}
...
func resolveStyle(tag string) (rgbColor, rgbColor, bool) {
	style, ok := tagStyleLookup[strings.ToLower(tag)]
	if !ok {
		return rgbColor{}, rgbColor{}, false
	}
	bg := mustColor(style.BG)
	return foregroundFor(bg, style.FG), bg, true
}

func printColored(text string, fg, bg rgbColor) {
	fmt.Printf("\x1b[1m\x1b[38;2;%d;%d;%dm\x1b[48;2;%d;%d;%dm%s\x1B[00m", fg.R, fg.G, fg.B, bg.R, bg.G, bg.B, text)
}

func renderTag(tag string) {
	if fg, bg, ok := resolveStyle(tag); ok {
		printColored(fmt.Sprintf(".%s.", tag), fg, bg)
	} else if !strings.ContainsRune(tag, '/') {
		printColored(fmt.Sprintf(".%s.", tag), rgbColor{R: 255, G: 120, B: 0}, rgbColor{R: 255, G: 255, B: 255})
	} else {
		fmt.Printf(".%s.", tag)
	}
}
